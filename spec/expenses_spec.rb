...
describe 'Finances' do
  ...
  describe 'Budget Items' do
    ...
    it 'to_h' do
      ...
      expect(item.to_h).to eq({"name" => "test", "amount" => 100, "rate_of_increase" => INFLATION_RATE, "duration" => :infinite})
    end
  end
      ...
      it 'prepares for csv conversion properly' do
        ...
        expect(csvs).to eq({0 =>{ 
                                "items"=>[{"name"=>"kids", "amount"=>-100, "rate_of_increase"=>0.03, "duration"=>:infinite},
                                          {"name"=>"income", "amount"=>200, "rate_of_increase" => 0.03, "duration"=>:infinite}],
                                },
                            3 => {"items"=>[{"name"=>"kids", "amount"=>-100, "rate_of_increase"=>0, "duration"=>1}]}
        })
      end
    end
  end
